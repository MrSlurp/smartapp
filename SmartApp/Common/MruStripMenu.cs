...
namespace SmartApp
{
	...
	/// <para>The class will optionally load the last set of files from the File
	...
	{
        private const string MRU_FILES_SECTION = "MRU_Files";
        private const string MRU_FILE_ITEM = "File_{0}";
        private const string MRU_MAX_FILE = "count";
		...
		/// <param labelName="recentFileMenuItem">The temporary menu item which will be replaced with the MRU list.</param>
		/// <param labelName="clickedHandler">The delegate to handle the item selection (click) event.</param>
		/// <param labelName="FileName"></param>
		public MruStripMenu(ToolStripMenuItem recentFileMenuItem, ClickedHandler clickedHandler, string FileName)
			: this(recentFileMenuItem, clickedHandler, FileName, true, 4)
		{
		}
		...
		/// <param labelName="recentFileMenuItem">The temporary menu item which will be replaced with the MRU list.</param>
		/// <param labelName="clickedHandler">The delegate to handle the item selection (click) event.</param>
		/// <param labelName="FileName">The name or path of the File key to use to store the MRU list and settings.</param>
		/// <param labelName="maxEntries">The maximum number of items on the MRU list.</param>
		public MruStripMenu(ToolStripMenuItem recentFileMenuItem, ClickedHandler clickedHandler, string FileName, int maxEntries)
			: this(recentFileMenuItem, clickedHandler, FileName, true, maxEntries)
		{
		}
		...
		/// <param labelName="clickedHandler">The delegate to handle the item selection (click) event.</param>
		/// <param labelName="FileName">The name or path of the File key to use to store the MRU list and settings.</param>
		/// <param labelName="loadFromFile">Loads the MRU settings from the File immediately.</param>
		public MruStripMenu(ToolStripMenuItem recentFileMenuItem, ClickedHandler clickedHandler, string FileName, bool loadFromFile)
			: this(recentFileMenuItem, clickedHandler, FileName, loadFromFile, 4)
  		{
		}
		...
		/// <param labelName="loadFromFile">Loads the MRU settings from the File immediately.</param>
		/// <param labelName="maxEntries">The maximum number of items on the MRU list.</param>
		public MruStripMenu(ToolStripMenuItem recentFileMenuItem, ClickedHandler clickedHandler, string FileName, bool loadFromFile, int maxEntries)
		{
			Init(recentFileMenuItem, clickedHandler, FileName, loadFromFile, maxEntries);
		}

		protected void Init(ToolStripMenuItem recentFileMenuItem, ClickedHandler clickedHandler, string FileName, bool loadFromFile, int maxEntries)
		{
...
//					MenuItems[i].Text = FixupEntryname(startNumber, MenuItems[i].Text.Substring(startNumber == 10 ? 5 : 4));
		...
		public void LoadFromFile(string keyName)
		{
			FileName = keyName;
			LoadFromFile();
		}
		...
		public void LoadFromFile()
		{
			if (FileName != null)
			{
				...
				RemoveAll();
                IniFileParser iniFile = new IniFileParser(FileName);

                maxEntries = int.Parse(iniFile.GetValue(MRU_FILES_SECTION, MRU_MAX_FILE, maxEntries));

				for (int number = maxEntries; number > 0; number--)
				{
					string filename = iniFile.GetValue(MRU_FILES_SECTION, string.Format(MRU_FILE_ITEM, number));
		...
		public void SaveToFile()
		{
			...
			{
                IniFileParser iniFile = new IniFileParser(FileName);
                iniFile.SetValue(MRU_FILES_SECTION, MRU_MAX_FILE, maxEntries.ToString());
				int number = 1;
				int i = StartIndex;
				for (; i < EndIndex; i++, number++)
				{
                    iniFile.SetValue(MRU_FILES_SECTION, string.Format(MRU_FILE_ITEM, number), ((MruMenuItem)MenuItems[i]).Filename);
				}
                iniFile.Save();
			}
		}
	...
	public class MruStripMenuInline : MruStripMenu
	{
		...
		public MruStripMenuInline(ToolStripMenuItem owningMenu, ToolStripMenuItem recentFileMenuItem, ClickedHandler clickedHandler, string FileName)
			: this(owningMenu, recentFileMenuItem, clickedHandler, FileName, true, 4)
		{
		}

		public MruStripMenuInline(ToolStripMenuItem owningMenu, ToolStripMenuItem recentFileMenuItem, ClickedHandler clickedHandler, string FileName, int maxEntries)
			: this(owningMenu, recentFileMenuItem, clickedHandler, FileName, true, maxEntries)
		{
		}

		public MruStripMenuInline(ToolStripMenuItem owningMenu, ToolStripMenuItem recentFileMenuItem, ClickedHandler clickedHandler, string FileName, bool loadFromFile)
			: this(owningMenu, recentFileMenuItem, clickedHandler, FileName, loadFromFile, 4)
		{
		}

		public MruStripMenuInline(ToolStripMenuItem owningMenu, ToolStripMenuItem recentFileMenuItem, ClickedHandler clickedHandler, string FileName, bool loadFromFile, int maxEntries)
		{
