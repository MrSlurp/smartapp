...
namespace SmartApp
{
    ...
    public class BTDoc : Object
    {
        ...
        ~BTDoc()
        {
            TraiteMessage(MESSAGE.MESS_CMD_STOP, null);
        }
        ...
        // Return: /
        //*****************************************************************************************************
        public void TraiteMessage(MESSAGE Mess, object Param)
        {
            switch (Mess)
            {
                ...
                case MESSAGE.MESS_CMD_STOP:
                    GestData.TraiteMessage(Mess, Param);
                    GestScreen.TraiteMessage(Mess, Param);
        ...
        // Return: /
        //*****************************************************************************************************
        public bool ReadConfigDocument(string strFullFileName)
        {
            ...
            while (Node != null)
            {
                ...
                switch (Id)
                {
                    case XML_CF_TAG.DataSection:
                        if (!this.GestData.ReadIn(Node))
                            return false;
                        break;
                    case XML_CF_TAG.TrameSection:
                        if (!this.GestTrame.ReadIn(Node))
                            return false;
                        break;
                    case XML_CF_TAG.ScreenSection:
                        if (!this.GestScreen.ReadIn(Node))
                            return false;
                        break;
                    case XML_CF_TAG.FileHeader:
                        if (!this.ReadFileHeader(Node))
                            return false;
                        break;
                    case XML_CF_TAG.Program:
                        if (!this.GestFunction.ReadIn(Node)
                            || !this.GestTimer.ReadIn(Node)
                            || !this.GestLogger.ReadIn(Node)
                            )
                            return false;
                        break;
                    default:
        ...
        // Return: /
        //*****************************************************************************************************
        protected bool ReadFileHeader(XmlNode NodeHeader)
        {
                    ...
                    case XML_CF_TAG.FileVersion:
                        ...
                        if (Cste.CUR_FILE_VERSION.ToString() != SubHeaderNode.Value)
                        {
                            ...
                            if (AttrIndice != null && int.TryParse(AttrIndice.Value, out FileVer))
                            {
                                ...
                                {
                                    // fichier avec une version plus récente...
                                    return false;
                                }
                            }
                            else
