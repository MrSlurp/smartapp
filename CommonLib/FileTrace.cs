...
{
	/// <summary>
	/// Description résumée de CTraces.
	/// </summary>
	public static class FileTrace
	{
		// Niveaux de trace
		...
		private const int TAILLEMAX_MESSAGE1	= 20;
		
		// Nombre de jours de logs conservés
		private const int NBR_JOURS = 7 ;
		...
		public static void Initialize(int iTracesLevel)
		{
			sLogDirectory = Application.StartupPath ;
			sLogFileName = "Trace.txt" ;
				...
				lock(oSectionCritique)
				{
					...
					if(LogDirectory(out sFileName))
					{
						...
						try
						{
							...
							TxtLog.WriteLine(oDHSysteme.ToString("dd")+"/"+oDHSysteme.ToString("MM")+
