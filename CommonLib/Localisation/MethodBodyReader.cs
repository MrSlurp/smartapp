...
namespace CommonLib
{
    public class MethodBodyReader
    {
        ...
        public string GetBodyCode(string SearchText)
        {
            string result = "";
            if (instructions != null)
            {
                for (int i = 0; i < instructions.Count; i++)
                {
                    string mline = instructions[i].GetCode();
                    if (mline.ToLower().Contains(SearchText))
                    {
                        result += instructions[i].GetCode() + Environment.NewLine;
                    }
                }
        ...
        public string GetBodyCode(string SearchText1, string SearchText2)
        {
            string result = "";
            string old = "";
            if (instructions != null)
            {
                ...
                {
                    string mline = instructions[i].GetCode();
                    if (mline.ToLower().Contains(SearchText1))
                    {
                        old = instructions[i].GetCode();
                    }
                    else if (mline.ToLower().Contains(SearchText2))
                    {
                        result += old + Environment.NewLine;
                        result += instructions[i].GetCode() + Environment.NewLine;
                    }
                }
            }
